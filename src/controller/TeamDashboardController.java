...
public class TeamDashboardController extends Controller<League> {
    ...
    private void initialize() {
        ...
        playerTable.getSelectionModel().selectedItemProperty().addListener((obs, old, nw) -> unsignPlayerButton.setDisable(nw == null));
    ...
    private String getActivePlayerTool(int x) {
        try {
            if (currentTeam[x] == null) {return "Unallocated";}
            else {return currentTeam[x].toString();}
        }
    ...
    private void handlePlayerUnsign() {
        /*Debugging*/ System.out.println("Unsign button clicked.");
        Team team = model.getLoggedInManager().getTeam();
        ...
        switch(playerState) {
            ...
            case 1:
                ...
                break;
            default:
                showError("Something isn't right...", "??????");
        }
    }
